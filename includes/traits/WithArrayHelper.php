...
trait WithArrayHelper {
     ...
     * @returns array|false
     */
    protected function array_property( $key ){
        if ( is_string( $key ) ){
     ...
     * @return array|false
     */
    protected function &array_path( &$root, $path ){
        $target = &$root;
        if ( is_array( $path ) ){
     ...
     * @return mixed|null
     */
    protected function &array_get( &$fromArray, $key, $default = null ){
        $property = $this->array_property( $key );
        if ( $property !== false ){
            $path = &$this->array_path( $fromArray, $property[ 'path' ] );
     ...
     */
    protected function array_set( &$toArray, $key, $value ){
        $property = $this->array_property( $key );
        if ( $property !== false ) {
            $path = &$this->array_path( $toArray, $property[ 'path' ] );
            if ( $path !== false && array_key_exists( $property[ 'key' ], $path ) ){
